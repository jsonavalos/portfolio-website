...
var courses_link = document.getElementById("courses-link");
var links = [contact_link, projects_link, internships_link, skills_link, courses_link];
var drawerElements = document.getElementsByClassName("drawer")
var expandAllDrawersBtn = document.getElementById("expand-all-drawers-btn");
var areDrawersExpanded = false;
...
for (let i = 0; i < drawerElements.length; i++) {
    drawerElements[i].addEventListener('click', (event) => {
        ...
        if (currentStyleDisplay == "block") {
            ...
            applyUnhoverEffect(i);
        }
    });
}


expandAllDrawersBtn.addEventListener('click', (event) => {
    for (let i = 0; i < drawerElements.length; i++) {
        if (areDrawersExpanded) {
            drawerElements[i].getElementsByTagName('div')[0].style.display = "none";
            expandAllDrawersBtn.innerHTML = "<strong>+</strong> Expand All";
        }
        else {
            drawerElements[i].getElementsByTagName('div')[0].style.display = "block";
            applyUnhoverEffect(i);
            expandAllDrawersBtn.innerHTML = "<strong>-</strong> Collapse All";
        }
    }
    areDrawersExpanded = !areDrawersExpanded;
})

function applyUnhoverEffect(i) {
    drawerElements[i].style.backgroundColor = "white";
    drawerElements[i].style.color = "black";
}
